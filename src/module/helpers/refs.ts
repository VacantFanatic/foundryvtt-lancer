...
export function handleRefClickOpen(html: JQuery) {
  const maybe = html as unknown as { jquery?: string; 0?: unknown; length?: unknown; constructor?: { name?: string } };
  const $root = maybe?.jquery ? (html as JQuery) : $(html as unknown as HTMLElement);
  try {
